...
public class MostFrequentlyOccuringCharacter {
        ...
        for (String word : words) {
            ...
            if (word.length() % 2 != 0) {
                ...
                for (char c : word.toCharArray()) {
                    int count = Math.toIntExact(word.chars().filter(f -> f == c).count());

                    if (count > maxCount) {
                        maxCount = count;
                        mostFrequent = Character.toLowerCase(c);
                    }
                }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(mostFrequentlyOccuringCharacter.oddNumsOfChars("Bravo! You've just successfully navigated through"));
    }
