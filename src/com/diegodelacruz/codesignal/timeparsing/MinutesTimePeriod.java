package com.diegodelacruz.codesignal.timeparsing;

import java.time.LocalTime;

/**
 * You are given a time period formatted as a string in the HH:MM:SS - HH:MM:SS format.
 * HH:MM:SS represents the time in hours, minutes, and seconds form, and the hyphen (-)
 * separates the start time from the end time of the period.
 * Your task is to calculate how many minutes pass from the start time until the end time.
 */
public class MinutesTimePeriod {

    /**
     * Guidelines:
     * - The input times are always valid time strings in the HH:MM:SS format, with HH ranging from 00 to 23, and MM and SS from 00 to 59.
     * - The output should be an integer, representing the total length of the time period in minutes.
     * - The start time of the period will always be earlier than the end time, so periods that cross over midnight (like 23:00:00 - 01:00:00) are not considered.
     * - We are interested in the number of times the time passes some HH:MM:00 after the start time until the end time. Any remaining seconds should be disregarded;
     * for instance, a period of "12:15:00 - 12:16:59" represents 1 minute, not 2, and a period of "12:14:59 - 12:15:00" also represents 1 minute.
     *
     * @param timePeriod
     * @return
     */
    public int timePeriodLength(String timePeriod) {
        // TODO: implement the function
        /* First approach - Personal Approach
        String[] firstPeriod = timePeriod.substring(0, 8).split(":");
        String[] secondPeriod = timePeriod.substring(11).split(":");
        int count = 0;


        int firstHours = Integer.parseInt(firstPeriod[0]);
        int firstMins = Integer.parseInt(firstPeriod[1]);
        int secHours = Integer.parseInt(secondPeriod[0]);
        int secMins = Integer.parseInt(secondPeriod[1]);

        if (firstHours == secHours) {
            return secMins - firstMins;
        } else if (firstHours < secHours || firstMins < secMins) {
            int hoursDiff = secHours - firstHours;
            int minDiff = hoursDiff * 60;
            count = (minDiff - firstMins) + secMins;
            return count;
        } else {
            return count;
        }

         */

        // Enhanced version using LocalTime and toSecondOfDay to calculate how many seconds exist in the time provided
        String [] parts = timePeriod.split(" - ");
        int startMinuteIndex = LocalTime.parse(parts[0]).toSecondOfDay() / 60;
        int endMinuteIndex = LocalTime.parse(parts[1]).toSecondOfDay() / 60;
        return endMinuteIndex - startMinuteIndex;

    }

    /**
     * Where timePeriod is a string formatted as HH:MM:SS - HH:MM:SS.
     * The function should return a single integer that represents the total length of the specified time period in minutes.
     * <p>
     * Example:
     * timePeriodLength("12:15:30 - 14:00:00");
     * // should return 105
     * Another example ("00:00:00 - 23:59:59")
     * // should return 1439
     *
     * @param args
     */
    public static void main(String[] args) {
        MinutesTimePeriod minutesTimePeriod = new MinutesTimePeriod();
        System.out.println(minutesTimePeriod.timePeriodLength("12:15:30 - 14:00:00"));
    }

}
